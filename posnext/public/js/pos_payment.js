...
posnext.PointOfSale.Payment = class {
	...
	make_invoice_fields_control() {
		...
		// Get fields from POS Settings first
		frappe.db.get_doc("POS Settings", undefined).then((doc) => {
			const pos_fields = doc.invoice_fields || [];
			const custom_fields = [];
			
			// Add custom fields based on constructor settings
			...
			// Combine POS Settings fields with custom fields
			const all_fields = [...pos_fields, ...custom_fields];
			
			if (!all_fields.length) return;
	...
	bind_events() {
		...
		// Changed from 'POS Invoice' to 'Sales Invoice'
		...
		frappe.ui.form.on('Sales Invoice', 'coupon_code', (frm) => {
			if (frm.doc.coupon_code && !frm.applying_coupon_code) {
				if (!frm.doc.ignore_pricing_rule) {
					frm.applying_coupon_code = true;
					frappe.run_serially([
						...
						() => (frm.applying_coupon_code = false)
					]);
	...
	update_totals_section(doc) {
		...
		// Branch value handling from second code
