...
posnext.PointOfSale.Payment = class {
    constructor({ events, wrapper, settings }) {
        ...
        this.reqd_invoice_fields = []; // Initialize required fields array
    ...
    make_invoice_fields_control() {
        const me = this;
        this.reqd_invoice_fields = []; // Ensure array is initialized
        frappe.db.get_doc("POS Settings", null).then((doc) => {
            let fields = doc.invoice_fields || [];
            ...
            fields.forEach(df => {
                ...
                let df_events = {
                    onchange: function() {
                        ...
                        } else if (this.df.fieldname === 'custom_credit_sales') {
                            if (this.get_value()) {
                                ...
                                me.current_payments.forEach(p => {
                                    if (p.mode_of_payment === me.default_payment) {
                                        const mode = me.sanitize_mode_of_payment(p.mode_of_payment);
                                        if (me[`${mode}_control`]) {
                                            me[`${mode}_control`].set_value(frm.doc.grand_total || 0);
                                        }
                                    }
                                });
                            }
            ...
            frappe.show_alert({
                message: __("Failed to load POS Settings. Please try again."),
                indicator: "red"
            });
        });
    }
    ...
    bind_events() {
        ...
        this.$payment_modes.on('click', '.mode-of-payment', function(e) {
            ...
            if (mode_clicked.hasClass('border-primary')) {
                ...
                me.selected_mode = me[`${mode}_control`] || null;
                if (me.selected_mode) {
        ...
        this.$component.on('click', '.submit-order-btn', () => {
            ...
            if (doc.custom_credit_sales && this.custom_show_credit_sales) {
        ...
        frappe.ui.form.on('Sales Invoice', 'paid_amount', (frm) => {
            ...
            if (!is_cash_shortcuts_invisible) {
    ...
    toggle_remarks_control() {
        if (!this.$totals_section.find('.remarks').length) {
            this.$totals_section.append('<div class="remarks"></div>');
        }
    ...
    focus_on_default_mop() {
        ...
        payments.forEach(p => {
            ...
            if (p.default) {
                setTimeout(() => {
                    const $mode = this.$payment_modes.find(`.${mode}.mode-of-payment-control`);
                    if ($mode.length) {
                        $mode.parent().click();
                    }
                }, 500);
    ...
    update_totals_section(doc) {
        ...
        const branch_value = $('.input-with-feedback[data-fieldname="branch"]').val();
        if (branch_value) {
    ...
    validate_reqd_invoice_fields() {
        ...
        for (const field of this.reqd_invoice_fields) {
